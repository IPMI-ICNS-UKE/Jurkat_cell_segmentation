...
measured with Fura Red (red) emission to segmen existing objects
(T-cells). """
...
base_dir_green = str(root_dir_green + '/Training/')
base_dir_red = str(root_dir_red + '/Training/')
sequence_dir_green = str(base_dir_green) + '01/'
sequence_dir_red = str(base_dir_red) + '01/'
target_dir = str(base_dir_green) + 'GT_01/'
print(root_dir_green)
    ...
    for seq in range(len(sequence_dirs)):
        ...
        for frame in range(0, len(fnames)):
            ...
            if (seq == frame == 0):
                ...
                ColIm = np.reshape(corrected_img, (-1, 1))
                RowIm = np.reshape(np.transpose(corrected_img), (-1, 1))
...
def frame_reconstruct(y_trgt, y_prdct, nRow, nCol):
    ...
    tg_output = np.reshape(gt[-numFrame*nRow*nCol-1:-1],
                           (numFrame, nRow, nCol)).astype(np.float64)
    pr_output = np.reshape(pred[-numFrame*nRow*nCol-1:-1],
                           (numFrame, nRow, nCol)).astype(np.float64)
...
# save the model
import pickle
with open('trainedESN_Red.pickle', 'wb') as handle:
